...
class DomainEngine:
    ...
    def store_domains(self, domain):
        ...
        if domain.empty:
            ...
            query = """SELECT _vid_, _cid_, _tid_, attribute, a.rv_val, a.val_id
                       FROM {} , unnest(string_to_array(regexp_replace(domain,\'[{\"\"}]\',\'\',\'gi\'),\'|||\'))
                       WITH ORDINALITY a(rv_val,val_id)""".format(AuxTables.cell_domain.name)
